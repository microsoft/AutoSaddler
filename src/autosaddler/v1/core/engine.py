...
from autosaddler.v1.core.callbacks import (
    BudgetUpdatedEvent,
...
from autosaddler.v1.core.state import EvaluationCache, FrontierType, GEPAState, ValsetEvaluation, initialize_gepa_state
from autosaddler.v1.logging.experiment_tracker import ExperimentTracker
from autosaddler.v1.logging.logger import LoggerProtocol
from autosaddler.v1.logging.utils import log_detailed_metrics_after_discovering_new_program
...
from autosaddler.v1.utils import StopperProtocol
...
class GEPAEngine(Generic[DataId, DataInst, Trajectory, RolloutOutput]):
    ...
    def __init__(
        ...
        reflective_proposer: ProposeNewCandidate,
    ...
    def run(self) -> GEPAState[RolloutOutput, DataId]:
        ...
        if self.display_progress_bar:
            ...
            if total_calls is not None:
                progress_bar = tqdm(total=total_calls, desc="AutoSaddler", unit="rollouts")
            else:
                progress_bar = tqdm(desc="AutoSaddler", unit="rollouts")
        ...
        # Run final reflection for the last iteration (deferred Session 2)
        if hasattr(self.reflective_proposer, 'finalize'):
            try:
                self.reflective_proposer.finalize(state)
            except Exception as e:
                self.logger.log(f"finalize() failed (non-fatal): {e}")
                self.logger.log(traceback.format_exc())
