...
class EvaluationBatch(Generic[Trajectory, RolloutOutput]):
    ...
    - outputs: raw per-example outputs from upon executing the candidate. AutoSaddler does not interpret these;
      they are forwarded to other parts of the user's code or logging as-is.
    - scores: per-example numeric scores (floats). AutoSaddler sums these for minibatch acceptance
...
class GEPAAdapter(Protocol[DataInst, Trajectory, RolloutOutput]):
    ...
    and the AutoSaddler optimization engine. Implementers provide three responsibilities:

    The following are user-defined types that are not interpreted by AutoSaddler but are used by the user's code
    ...
    3) Optional instruction proposal (propose_new_texts):
       AutoSaddler provides a default implementation (instruction_proposal.py) that serializes the reflective dataset
    ...
    - scores: higher is better. AutoSaddler uses:
    ...
    - trajectories: opaque to AutoSaddler (the engine never inspects them). They must be
    ...
    def evaluate(
        ...
        - EvaluationBatch with:
          - outputs: raw per-example outputs (opaque to AutoSaddler).
    ...
    def make_reflective_dataset(
        ...
        - components_to_update: subset of component names for which the proposer has
          requested updates. At a time, AutoSaddler identifies a subset of components to update.
